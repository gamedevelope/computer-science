...
((lambda ()
   ...
   (define (representations-for-complex-numbers)
     ;     (define (real-part z) (* magnitude z) (cos (angle z)))
     ;     (define (imag-part z) (* (magnitude z) (sin (angle z))))
     (define (mignitude z) (car z))
     ...
     (println (real-part (make-from-real-imag 1 2)))
     (println (imag-part (make-from-real-imag 1 2)))
     '())
   (link 'representations-for-complex-numbers representations-for-complex-numbers)
   (define (ex2.73)
   ...
   (define (ch2.5.1)
     ...
     (define (install-rational-package)
       ...
       (define (tag n) (attach-tag 'rational x))
   ...
   (define (ch2.5.3)
     ...
     (define =zero? (get '=zero? '()))
     (println (=zero? 0))
     (define l1 (adjoin-term (make-term 3 1)
                             (adjoin-term (make-term 2 4)
                                          '())))
     (define l2 (adjoin-term (make-term 3 5)
                             (adjoin-term (make-term 2 14)
                                          '())))
     (println l1)
     (println l2)
     (println (add-terms l1 l2))
     (println (negative l1))
     (println (add-terms l1 (negative l2)))
     (define poly1 (make-polynomial 'x l1))
     (define poly2 (make-polynomial 'x l2))
     (define poly3 (make-polynomial 'y l2))
     (println poly1)
     (println (add poly1 poly3))
     ;;; 2.89
     )
   (link 'ch2.5.3 ch2.5.3)
   ))
(last-exercise)
