...
(define (divider m1 m2 quotient)
  (define (process-new-value)
    (cond ((and (has-value? m2) (= (get-value m2) 0))
           (error "m2 must not be zero"))
...
(define (celsius-fahrenheit-converter-3.37 x)
  (c+ (c* (c/ (cv 9) (cv 5))
          x)
      (cv 32)))
(define C3.37 (make-connector))
(define F3.37 (celsius-fahrenheit-converter-3.37 C3.37))
