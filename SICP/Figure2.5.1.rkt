        ...
        (else
         (let ((types-procedure (car tower)))
           (let ((types (car types-procedure))
             ...
             (cond ((eq? type1 (car types))
                    (append (list procedure)
                            (if (eq? type2 (cdr types))
                                '()
                                (type-tower (cdr types) type2 (cdr tower)))))
                   ((eq? type2 (car types))
                    (append (list procedure)
                            (if (eq? type1 (cdr types))
                                '()
                                (type-tower (cdr types) type1 (cdr tower)))))
                   (else (type-tower type1 type2 (cdr tower)))))))))
